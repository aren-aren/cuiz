...
let bgVideoSource = document.getElementById("bg-video-source");
...
async function frm(id){
  try {
    const json = await fetch("list?member_ID="+id).then(result=>result.json())
    console.log(json)    
      bgVideoSource.setAttribute("src", json.bg)  
      for(let list of json.ar){
